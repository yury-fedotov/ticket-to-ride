...
def generate_connectionstyle_iterable(n_edges: int, rad: float = 0.2) -> tp.FrozenSet[str]:
    ...
    Returns:
        A frozenset of strings compatible with `connectionstyle` argument of ``nx.draw_networkx_edges()`` function.
    """
    ...
    match n_edges:
        case 0:
            return frozenset()
        case 1:
            return frozenset(("arc3,rad=0.0",))
        case 2:
            return frozenset(f"arc3,rad={sign}{rad}" for sign in _sign_defining_substrings)
        case _:
